...
def main():
    ...
    if data_set == "contexts":
        ...
        for ind, context_name in enumerate(context_names):
            if ind > 0 :
                break;
            path_to_input_file = "../../data/Vaccination/mothering_contexts/" + context_name + ".csv"
            path_to_output_file = "../../data/Vaccination/mothering_contexts/mothering_contexts_with_extractions/" + context_name + "_withExt.csv"
            data, header = load_data(path_to_input_file)
            df = pd.DataFrame(data, columns = header )
            df_res = evaluate_df(df.copy(), sent_label="sentence")
